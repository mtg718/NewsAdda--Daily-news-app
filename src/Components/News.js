...
if(props.category==='general'){
  document.title ="NewsAdda-Get Daily News";
}else{
  document.title = `${capital(props.category) + " -NewsAdda"}`;
}
...
useEffect( ()=> {
 ...
 // eslint-disable-next-line
  ...
  const fetchMoreData = async() => {
    ...
    return (
      <>
        <h1 style = {{fontSize: '45px', margin : '30px 0px' , marginTop:'80px'}} className = 'text-center' >NewsAdda -Top {capital(props.category)} Headlines</h1>
       {loading && <Spinner/>}
        <InfiniteScroll
